...
for(i=0; i<ILGIS1-1; i++) {SkPb[i]=1; Kirt[i]=127; Trmp[i]=0;}
...
kz=ez;
ez=dz;
dz=strtokf(NULL, " ", &newpos); if(dz==NULL) dz= (char*)"_";
...
for(int ij = j+strlen(&eilute[j])-1; ij>=j; ij--)
	...
	if(Kirt[ij]!=127)
		{
		if(Kirciuota == 0) {Kirciuota=1;} else {Kirt[ij]=127;} //paliekam tik viena kircio zenkla zodyje
		}
	}
...
Kirt[i]=(Kirt[i]==127) ? 1:(Kirt[i]==0) ? 2:(Kirt[i]==1) ? 4:8;
}
