...
int nuskaityti_signala()
{
	...
	// mmap DB read-only: quality-100 db.raw is 300+ MB, too big to fread into a
	// memory-limited Speech extension. mmap pages in lazily (identical bytes).
	signalas = (short *) mmap(NULL, (size_t)signalo_failo_dydis, PROT_READ, MAP_PRIVATE, fileno(signalo_failas), 0);
	if (signalas == MAP_FAILED)
	{
		signalas = NULL;
		fclose(signalo_failas);
	...
	// uzdarome signalo faila (mmap lieka galioti po fclose)
	fclose(signalo_failas);
